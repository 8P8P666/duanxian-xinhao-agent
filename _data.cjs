/**
 * 共用数据模块
 * 作用：统一封装 Gate 与 Hyperliquid 的公开行情接口（只读取，不涉及任何交易权限）
 * 输出：统一成 { t, o, h, l, c, v } 结构的K线数组，前端再做指标计算
 *
 * 重要说明：
 * 1. 只调用公开行情接口，不需要 API Key，不保存任何私钥，不涉及下单。
 * 2. Hyperliquid 的资金费率是「每小时」口径，Gate 是「8 小时」口径，
 *    两者不能直接对比，本模块统一换算成「年化」后再输出。
 * 3. ⚠️ data-direct.js 是本模块的浏览器版本，计算口径必须完全一致。
 *    修改其中一个的取数逻辑或换算方式时，务必同步修改另一个。
 */
...
/* ============================================================
 * 周期定义：只保留 Gate 与 Hyperliquid 双方都支持的周期
 * ============================================================ */
...
/** Hyperliquid 全市场永续合约行情快照（标记价、资金费率、持仓量、24小时名义成交额） */
async function hlAssetContexts() {
  ...
  universe.forEach((u, i) => {
    ...
    map.set(u.name, {
      ...
      openInterestUsd: openInterest !== null && markPx !== null ? openInterest * markPx : null,
...
/**
 * 取单个平台的行情与K线
 * @param {string} platform  gate | hyperliquid
 * @param {string} market    spot | perp（Hyperliquid 只有永续）
 * @param {string} symbol    Gate 用 BTC_USDT，Hyperliquid 用 BTC
...
async function getCrossReference({ symbol, market }) {
  const base = String(symbol || 'BTC_USDT').toUpperCase().replace('_USDT', '');
  try {
    const contexts = await hlAssetContexts();
