 ...
 *       也就是说，本地测通的逻辑，部署到 Netlify 后行为一致
 *
 * 目录结构说明（扁平结构）：
...
const BLOCKED_EXT = ['.md', '.toml', '.mjs', '.cjs'];
...
server.listen(PORT, HOST, () => {
  console.log('');
  console.log('  短线共振信号智能体 · 本地测试服务器已启动');
  console.log('  ------------------------------------------------');
  console.log(`  本地地址： http://${HOST}:${PORT}`);
  console.log('');
  console.log('  接口自检：');
  console.log(`  行情接口： http://${HOST}:${PORT}/api/market?platform=gate&market=perp&symbol=BTC_USDT&interval=15m`);
  console.log(`  币种接口： http://${HOST}:${PORT}/api/symbols?platform=gate&market=perp`);
  console.log('');
